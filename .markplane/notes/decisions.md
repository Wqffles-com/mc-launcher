...
- Selection order (TASK-9ik83): explicit `--java` → system JVM of exact required major (version JSON `javaVersion`, default 8 for pre-1.13) → auto-downloaded managed runtime of that major → nearest system JVM (smallest ≥ major; a JVM older than required is never chosen — it would only produce a cryptic `UnsupportedClassVersionError`) → error suggesting `mc-launcher java install <major>`.
- Detection (TASK-28y9j): JAVA_HOME, PATH, per-OS roots (Program Files\Java|Eclipse Adoptium|Zulu|Amazon Corretto|Microsoft + `~/.jdks`; macOS JVMs dirs + homebrew opt; `/usr/lib/jvm`, `/usr/java`, `/opt/java`, sdkman); major from the JVM `release` file, `java -version` fallback; stub layouts that hold the executable directly (e.g. Oracle `javapath`) are probed via the executable itself; dedup by canonical path.
